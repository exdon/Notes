...
@Component({
  selector: 'app-postit-friday',
  templateUrl: './postit-friday.component.html',
  styleUrls: ['./postit-friday.component.scss']
})
export class PostitFridayComponent implements OnInit {
  ...
  constructor(
    ...
    this.storedJson = JSON.parse(this.localStorageService.get('postit-friday'))
  ...
  setId() {
    const oldRecords = this.localStorageService.get('postit-friday');
  ...
  saveMarkdown(cardId: any) {
    try {
      this.localStorageService.set('postit-friday', JSON.stringify(this.notesSaved));
      this.success = true;
      this.idCard = cardId;
  ...
  deleteMarkdown(cardId: any) {
    ...
    if (oldRecords !== null) {
      try {
        ...
        this.localStorageService.set('postit-friday', JSON.stringify(cardList));
        this.success = true;
        this.idCard = cardId;
        ...
        setTimeout(() => {
          this.error = false;
          this.disabledFields = false;
          this.notesSaved = JSON.parse(this.localStorageService.get('postit-friday'));
        }, 5000)
      } catch (err) {
        this.error = true;
        this.idCard = cardId
