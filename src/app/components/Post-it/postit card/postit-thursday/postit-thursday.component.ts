...
@Component({
  selector: 'app-postit-thursday',
  templateUrl: './postit-thursday.component.html',
  styleUrls: ['./postit-thursday.component.scss']
})
export class PostitThursdayComponent implements OnInit {
  ...
  constructor(
    ...
    this.storedJson = JSON.parse(this.localStorageService.get('postit-thursday'))
  ...
  setId() {
    const oldRecords = this.localStorageService.get('postit-thursday');
  ...
  saveMarkdown(cardId: any) {
    try {
      this.localStorageService.set('postit-thursday', JSON.stringify(this.notesSaved));
      this.success = true;
      this.idCard = cardId;
  ...
  deleteMarkdown(cardId: any) {
    ...
    if (oldRecords !== null) {
      try {
        ...
        this.localStorageService.set('postit-thursday', JSON.stringify(cardList));
        this.success = true;
        this.idCard = cardId;
        ...
        setTimeout(() => {
          this.error = false;
          this.disabledFields = false;
          this.notesSaved = JSON.parse(this.localStorageService.get('postit-thursday'));
        }, 5000)
      } catch (err) {
        this.error = true;
        this.idCard = cardId
