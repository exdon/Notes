...
@Component({
  selector: 'app-postit-wednesday',
  templateUrl: './postit-wednesday.component.html',
  styleUrls: ['./postit-wednesday.component.scss']
})
export class PostitWednesdayComponent implements OnInit {
  ...
  constructor(
    ...
    this.storedJson = JSON.parse(this.localStorageService.get('postit-wednesday'))
  ...
  setId() {
    const oldRecords = this.localStorageService.get('postit-wednesday');
  ...
  saveMarkdown(cardId: any) {
    try {
      this.localStorageService.set('postit-wednesday', JSON.stringify(this.notesSaved));
      this.success = true;
      this.idCard = cardId;
  ...
  deleteMarkdown(cardId: any) {
    ...
    if (oldRecords !== null) {
      try {
        ...
        this.localStorageService.set('postit-wednesday', JSON.stringify(cardList));
        this.success = true;
        this.idCard = cardId;
        ...
        setTimeout(() => {
          this.error = false;
          this.disabledFields = false;
          this.notesSaved = JSON.parse(this.localStorageService.get('postit-wednesday'));
        }, 5000)
      } catch (err) {
        this.error = true;
        this.idCard = cardId
