...
@Component({
  selector: 'app-postit-monday',
  templateUrl: './postit-monday.component.html',
  styleUrls: ['./postit-monday.component.scss']
})
export class PostitMondayComponent implements OnInit {
  ...
  constructor(
    ...
    this.storedJson = JSON.parse(this.localStorageService.get('postit-monday'))
  ...
  setId() {
    const oldRecords = this.localStorageService.get('postit-monday');
  ...
  saveMarkdown(cardId: any) {
    try {
      this.localStorageService.set('postit-monday', JSON.stringify(this.notesSaved));
      this.success = true;
      this.idCard = cardId;
  ...
  deleteMarkdown(cardId: any) {
    ...
    if (oldRecords !== null) {
      try {
        ...
        this.localStorageService.set('postit-monday', JSON.stringify(cardList));
        this.success = true;
        this.idCard = cardId;
        ...
        setTimeout(() => {
          this.error = false;
          this.disabledFields = false;
          this.notesSaved = JSON.parse(this.localStorageService.get('postit-monday'));
        }, 5000)
      } catch (err) {
        this.error = true;
        this.idCard = cardId
